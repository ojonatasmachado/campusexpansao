/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
(function () {
  // Acentos por categoria (sempre dentro da paleta CE.X)
  const ACCENTS = {
    ocre: '#C0934E', argila: '#B07355', olive: '#7A9E3F', oliveDeep: '#4F6B26',
    pinho: '#4F7264', ardosia: '#5C7488',
  };

  // ── COR = ESTANTE (regra do sistema de banners) ──
  // A cor de acento NÃO é editável: é derivada da estante/nível.
  // Para ministrar  → cor por faixa etária
  // Para liderar     → Ardósia (família estrutural / ferramenta)
  const MINISTRAR_ACCENT = {
    'Juniores': ACCENTS.ocre,
    'Adolescentes': ACCENTS.argila,
    'Jovens': ACCENTS.olive,
    'Igreja toda': ACCENTS.pinho,
  };
  const LEVEL_ACCENT = {
    'Fundação': ACCENTS.ocre,
    'Liderança': ACCENTS.argila,
    'Multiplicação': ACCENTS.olive,
  };
  // Estantes válidas por família (alimentam o seletor do editor)
  const SHELVES = {
    'Para ministrar': ['Juniores', 'Adolescentes', 'Jovens', 'Igreja toda'],
  ...
  const ACCENT_NAME = {
    [ACCENTS.ocre]: 'Ocre', [ACCENTS.argila]: 'Argila', [ACCENTS.olive]: 'Oliva',
    [ACCENTS.pinho]: 'Pinho', [ACCENTS.ardosia]: 'Ardósia', [ACCENTS.oliveDeep]: 'Oliva profundo',
  };

  function accentFor(item) {
    if (!item) return ACCENTS.olive;
    if (item.type === 'material') return item.family === 'Para liderar' ? ACCENTS.ardosia : (MINISTRAR_ACCENT[item.shelf] || ACCENTS.olive);
    if (item.type === 'curso') return LEVEL_ACCENT[item.level] || ACCENTS.olive;
    if (item.type === 'evento') return ACCENTS.pinho;
    return ACCENTS.olive; // mentoria
  }

  // Compat: mapa antigo ainda referenciado abaixo
  const SHELF_ACCENT = new Proxy({}, { get: (_, shelf) => MINISTRAR_ACCENT[shelf] || ACCENTS.olive });
  ...
  function mat(family, shelf, code, title, desc, msgs, pages, price, views, status) {
    ...
    return {
      ...
      messageList: [],
      // Blocos de conteúdo (texto/pdf/slides) que geram os metadados.
      // Materiais semeados já vêm com uma peça que espelha messages/pages.
      contents: hasMsg
        ? [{ kind: 'word', name: 'Roteiro completo das mensagens', note: desc, chars: (pages || 1) * 1900, messages: msgs, pages, delivery: 'pdf', headings: [] }]
        : [{ kind: 'pdf', name: title, note: desc, file: `${title}.pdf`, pages }],
  ...
  const metrics = {
    ...
    origem: [
      ...
      { label: 'Direto', value: 19, color: ACCENTS.ocre },
      { label: 'Google', value: 13, color: ACCENTS.argila },
