...
> ok (ancorado/sticky) e o menu já inclui "Sua Vocação" (quiz) propositalmente. Manter a
> identidade CE.X (Oliva #7A9E3F, Cream #EDE6D3, Ink #0E110D, fonte Inter, marca `◆`, zero
> travessões `—`). Consulte `CEX_BrandBook_v3.html` para tokens.
...
- Não usar cinza/azul frio nos acentos. Não usar travessões `—`. Marcas: `◆ ◇ → →`.
